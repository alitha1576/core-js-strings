...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < splitSentence.length; i += 1) {
    if (splitSentence[i].length > longestWorld.length) {
      longestWorld = splitSentence[i];
    }
  }
  return longestWorld;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reversedWords = arr.map((word) => {
...
function extractNameFromTemplate(value) {
  ...
  return `${newValue[1]} ${newValue[2].slice(0, -1)}`;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (letter) => caesar[caesar.indexOf(letter) + 13]
  );
}
